...
Cacher.prototype.keyStr = function where1(options) {
    const _options = _.cloneDeep(options);
    if (!Object.prototype.hasOwnProperty.call(_options, "where") || _.isEmpty(options.where)) {
        return CircularJSON.stringify(_options, jsonReplacer);
    }
    let sql = this.sequelize.getQueryInterface().queryGenerator.selectQuery(this.modelName, _.pick(_options, ["where"], this.md));
    return CircularJSON.stringify(_.assign(_options, {where: sql.split("WHERE")[1].trim()}), jsonReplacer);
};
/**
 * Create redis key
 */
Cacher.prototype.key = function key(sql) {
    ...
    const str = self.keyStr(this.options);
